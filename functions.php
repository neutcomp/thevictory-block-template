...
function ah_breadcrumb_shortcode() {
    ob_start();
    ah_breadcrumb();
    return ob_get_clean();
}
add_shortcode( 'ah_breadcrumb', 'ah_breadcrumb_shortcode' );

function thevictory_contact_form_shortcode() {
    $errors = array();
    $values = array(
        'name' => '',
        'email' => '',
    ...
    if ( isset( $_POST['submitted'] ) ) {
        $values['name'] = sanitize_text_field( wp_unslash( $_POST['contactName'] ?? '' ) );
        $values['email'] = sanitize_email( wp_unslash( $_POST['email'] ?? '' ) );
        $values['comments'] = sanitize_textarea_field( wp_unslash( $_POST['comments'] ?? '' ) );

        if ( ! isset( $_POST['thevictory_contact_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['thevictory_contact_nonce'] ) ), 'thevictory_contact_form' ) ) {
            $errors['form'] = 'Dit formulier kon niet worden verzonden. Probeer het opnieuw.';
        }

        if ( '' === $values['name'] ) {
            $errors['name'] = 'Geef je naam op';
        }

        if ( '' === $values['email'] ) {
            $errors['email'] = 'Geef je emailadres op';
        } elseif ( ! is_email( $values['email'] ) ) {
            $errors['email'] = 'Het emailadres is ongeldig';
        }

        if ( '' === $values['comments'] ) {
            $errors['comments'] = 'Geef een bericht op';
        }

        $captcha_token = sanitize_text_field( wp_unslash( $_POST['g-recaptcha-response'] ?? '' ) );
        if ( empty( $errors ) && '' !== $captcha_token ) {
            $verification = wp_remote_post(
                'https://www.google.com/recaptcha/api/siteverify',
                array(
                    'body' => array(
                        'secret' => '6Ldo4WIiAAAAABWM3QCAe2sSqmAJc2hJngi9VV2l',
                        'response' => $captcha_token
                    )
                )
            );
            $verification_body = ! is_wp_error( $verification ) ? json_decode( wp_remote_retrieve_body( $verification ) ) : null;

            if ( ! empty( $verification_body->success ) ) {
                $email_to = get_bloginfo( 'admin_email' );
                $subject = 'Contactformulier: Bericht van ' . $values['name'];
                $body = "Naam: {$values['name']} \n\nEmail: {$values['email']} \n\nBericht: {$values['comments']}";
                $headers = 'From: ' . $values['name'] . ' <' . $email_to . ">\r\nReply-To: " . $values['email'];
                $email_sent = wp_mail( $email_to, $subject, $body, $headers );
            } else {
                $errors['captcha'] = 'Robot verification mislukt. Probeer opnieuw.';
            }
        } elseif ( empty( $errors ) ) {
            $errors['captcha'] = 'Vergeet niet de reCAPTCHA box aan te vinken';
        }
    }
    ...
    if ( $email_sent ) :
        ?>
        <div class="thanks"><p>Bedankt we zullen zo spoedig mogelijk iets van ons laten horen.</p></div>
        <?php
    else :
        if ( ! empty( $errors ) ) :
            ?><p class="error">Sorry een fout heeft opgetreden.</p><?php
        endif;
        ?>
        <form action="<?php echo esc_url( get_permalink() ); ?>" id="contactForm" method="post">
            <?php wp_nonce_field( 'thevictory_contact_form', 'thevictory_contact_nonce' ); ?>
            <ul class="contactform">
                <li>
                    <label for="contactName">Naam:</label>
                    <input type="text" name="contactName" id="contactName" value="<?php echo esc_attr( $values['name'] ); ?>" class="required requiredField" />
                    <?php if ( isset( $errors['name'] ) ) : ?><span class="error"><?php echo esc_html( $errors['name'] ); ?></span><?php endif; ?>
                </li>
                <li>
                    <label for="email">Email</label>
                    <input type="email" name="email" id="email" value="<?php echo esc_attr( $values['email'] ); ?>" class="required requiredField email" />
                    <?php if ( isset( $errors['email'] ) ) : ?><span class="error"><?php echo esc_html( $errors['email'] ); ?></span><?php endif; ?>
                </li>
                <li>
                    <label for="commentsText">Bericht:</label>
                    <textarea name="comments" id="commentsText" rows="20" cols="30" class="required requiredField"><?php echo esc_textarea( $values['comments'] ); ?></textarea>
                    <?php if ( isset( $errors['comments'] ) ) : ?><span class="error"><?php echo esc_html( $errors['comments'] ); ?></span><?php endif; ?>
                </li>
                <li>
...
add_shortcode( 'thevictory_contact_form', 'thevictory_contact_form_shortcode' );

function thevictory_meta_shortcode( $attributes ) {
    $attributes = shortcode_atts( array( 'key' => '' ), $attributes, 'thevictory_meta' );
    if ( '' === $attributes['key'] ) {
        return '';
    }

    return esc_html( get_post_meta( get_the_ID(), $attributes['key'], true ) );
}
add_shortcode( 'thevictory_meta', 'thevictory_meta_shortcode' );
